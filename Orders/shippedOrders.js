...
exports.ShippedOrders = class ShippedOrders {
  constructor(page) {
    this.page = page;
    ...
    this.shippedTab = page.getByRole('menuitem', { name: 'Shipped' }).first();
  }
  ...
  async gotoShippedOrders() {
    await expect(this.shippedTab).toBeVisible();
  ...
  async validateOnlyShippedStatusInMFHColumn() {
    // await expect(this.shippedTab).toBeVisible();
    // await this.shippedTab.click();
    await this.page.waitForLoadState("domcontentloaded");
    await this.page.waitForLoadState("networkidle");
